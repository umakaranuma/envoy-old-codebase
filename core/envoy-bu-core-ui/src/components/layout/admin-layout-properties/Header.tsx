...
export const Header = ({ handleMenuToggleClick, themeMode, sbClosed, appMenu }: { handleMenuToggleClick: Function; themeMode: 'light' | 'dark'; sbClosed: boolean; appMenu: MenuCategory[] }) => {
  return (
    <div className="app-header">
      ...
      <div className="main-header-container container-fluid d-flex align-items-center justify-content-between">
        ...
        <div className=" d-flex align-items-center">
          <HeaderContentRight themeMode={themeMode} />
        </div>
      </div>
...
const HeaderContentLeft = ({ handleMenuToggleClick, sbClosed, appMenu }: { handleMenuToggleClick: Function; sbClosed: boolean; appMenu: MenuCategory[] }) => {
  ...
  return (
    <div className="header-content-left">
      ...
      <div className="header-element ms-4 fs-14 d-none d-md-block">
            ...
            return (
              <React.Fragment key={index}>
                ...
                {isCustomItem ? (
                  ...
                  <div className="text-primary fw-semibold">{t(item.name)}</div>
                ) : index === 0 ? (
                  <div className="fw-semibold text-muted">{t(item.name)}</div>
                ) : (
                  <Link href={item.path || '#'} className="text-muted clickable-breadcrumb-text">
                    {t(item.name)}
                  </Link>
                )}
              </React.Fragment>
...
const HeaderContentRight = ({ themeMode }: { themeMode: 'light' | 'dark' }) => {
  ...
  //     const data = await getNotification('unread', '');
  ...
  //       setNotification(responseData.result.data);
