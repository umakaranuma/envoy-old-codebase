...
function Table({
  ...
  return (
    <>
      {enableTopContent && renderTopContent()}
      {Table}
      {tableInitiated && dataLength === 0 && <RecordNotFound />}
      {!tableInitiated && <TBodyLoader />}
      {recordControl && <RecordController tableProperties={tableProperties} isRowPerPageVisible={isRowPerPageVisible} isPaginationTextVisible={isPaginationTextVisible} />}
    </>
  );
}
