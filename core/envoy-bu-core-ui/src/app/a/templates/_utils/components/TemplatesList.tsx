...
import { fetchTemplatesTableData } from '../service';

function TemplatesList({ tableVers, onView, onEdit, handleOnDelete }: { tableVers: number; onView: Function; onEdit: Function; handleOnDelete: Function }) {
  ...
  const columns = useMemo<ITablePropertyColumn[]>(
    () => [
      ...
      },
      {
        ...
        accessorKey: 'type',
        sort: true,
        cell: ({ cell }: { cell: any }) => <div>{cell.type === 'single_form' ? 'Single Step' : 'Multi Step'}</div>,
      },
      {
        header: t('action'),
        align: 'center',
  ...
  const tableProperties = useAsyncTable({
    ...
    loadData: (props: any) => fetchTemplatesTableData(props),
    paginate: true,
    rowSelection: true,
    rowSelectionProp: {
      key: 'id',
      mode: 'single',
      action: (id: string) => router.push(`/a/templates/${id}`),
    },
    customState: {
      initState: {
        filters: {},
  ...
  return (
    <>
      <div className={`data-table-container card custom-card ${isFullscreen ? 'dtc-fullscreen card-fullscreen' : 'mt-4'}`}>
        <Table heading={<PageHeading title={t('templates')} icon="sun-light" />} {...{ tableProperties, isFullscreen, setIsFullscreen, setIsCustColumnVisible }} />
      </div>
      <CustomizeColumn
        key={tableColumnVers}
