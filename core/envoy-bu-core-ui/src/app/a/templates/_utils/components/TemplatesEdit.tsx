...
import { initFormData, ITemplate } from '../model';
import { getOneTemplate, updatetemplate } from '../api-service';
import { clearError, printError } from '@/helpers/handlers/validationErrorHandler';

export const TemplatesEdit = ({ isOpen, editId, afterUpdate, onCancel }: { isOpen: boolean; editId: string; onCancel: Function; afterUpdate: Function }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responseData = await getOneTemplate(editId);

      if (responseData?.is_success) {
        const data: ITemplate = responseData.result.template;
        onFormChange('title', data.name);
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const responseData = await updatetemplate(editId, formData);
        ...
        <ModalBody>
          <div className="row">
            ...
            <div className="col-12 mb-3">
              ...
              {skeleton ? <InputSkeleton /> : <Input type="textarea" value={formData?.description || ''} onChange={(e) => onFormChange('description', e.target.value)} name="description" rows={3} />}
            </div>
          </div>
        </ModalBody>
        <ModalFooter>
