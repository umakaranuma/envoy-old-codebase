...
import { initFormData, IJobtitle } from '../model';
import { getOneJobTitle, updateJobTitle } from '../api-service';
import { InputSkeleton } from '@/components/others/InputSkeleton';
import { useTrans } from '@/helpers/services/lang/langService';
import { clearError, printError } from '@/helpers/handlers/validationErrorHandler';

export const JobTitlesEdit = ({ isOpen, editId, afterUpdate, onCancel }: { isOpen: boolean; editId: string; onCancel: Function; afterUpdate: Function }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const responseData = await getOneJobTitle(editId);
      if (responseData?.is_success) {
        const data: IJobtitle = responseData.result;
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const responseData = await updateJobTitle(editId, formData);
  ...
  return (
    <Modal isOpen={isOpen} onBackdrop={() => onCancel()}>
      <ModalHeader title={t('edit_job_title', { entity: t('job_titles') })} onClose={() => onCancel()} />
