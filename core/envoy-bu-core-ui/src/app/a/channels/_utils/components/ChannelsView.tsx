...
import { getOneChannels } from '../api-service';
import { IChannels } from '../model';

export const ChannelsView = ({ isOpen, viewId, onClose, setEditId }: { isOpen: boolean; viewId: string; onClose: Function; setEditId: Function }) => {
  const t = useTrans('label.channels,otr.common');
  const [data, setData] = useState({} as IChannels);
  const [skeleton, setSkeleton] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      const responseData = await getOneChannels(viewId);
