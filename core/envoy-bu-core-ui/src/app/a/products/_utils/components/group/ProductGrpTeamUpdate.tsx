...
import { updateProductGroupTeam } from '../../api-service';
import { AsyncSelect } from '@apptimus-ui/select';
import { fetchAllTeamsDropdown } from '../../services';

export const ProductGrpTeamUpdate = ({ isOpen, editId, afterUpdate, onCancel }: { isOpen: boolean; editId: string; onCancel: Function; afterUpdate: Function }) => {
  const t = useTrans('label.products,otr.common');
  const tBe = useTrans('be.msg,be.error,be.attri');
  const [isFormProcessing, setIsFormProcessing] = useState(false);
  const [formData, setFormData] = useState({ team_ids: [] });
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const responseData = await updateProductGroupTeam(editId, formData);
