...
import { fetchRolesTableData } from '../services';
import RolesFilter from './RolesFilter';
import PageHeading from '@/components/others/PageHeading';
import { useTrans } from '@/helpers/services/lang/langService';
import DeleteConfirmPop from '@/components/others/DeleteConfirmPop';

function RolesList({ tableVers, onView, onEdit, handleOnDelete }: { tableVers: number; onView: Function; onEdit: Function; handleOnDelete: Function }) {
  ...
  const columns = useMemo<ITablePropertyColumn[]>(
    () => [
      ...
      // {
      //   id: 'number_of_privileges',
      //   header: t('number_of_privileges'),
      //   accessorKey: 'number_of_privileges',
      //   sort: true,
      //   align: 'right',
      // },
      // {
      //   id: 'number_of_assigned_users',
      //   header: t('number_of_assigned_users'),
      //   accessorKey: 'number_of_assigned_users',
      //   sort: true,
      //   align: 'right',
      // },
      {
  ...
  return (
    <>
      <div className={`data-table-container card custom-card ${isFullscreen ? 'dtc-fullscreen card-fullscreen' : 'mt-4'}`}>
        <Table heading={<PageHeading title={t('roles')} icon="sun-light" />} {...{ tableProperties, isFullscreen, setIsFullscreen, setIsCustColumnVisible }} />
      </div>
      <CustomizeColumn
        key={tableColumnVers}
