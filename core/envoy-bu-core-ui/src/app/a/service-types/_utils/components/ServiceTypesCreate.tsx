...
function ServiceTypesCreate({ isOpen, onCancel, afterSave }: { isOpen: boolean; onCancel: Function; afterSave: Function }) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    clearError(form.service_type.store);
    setIsFormProcessing(true);

    try {
      const responseData = await createServiceType(formData);
      setIsFormProcessing(false);

      if (responseData.status_code === 417) {
        printError(responseData.result, form.service_type.store, tBe);
      }
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalHeader title={t('add_service_type', { entity: t('service_types') })} onClose={() => onCancel()} />
      <form onSubmit={onSubmit} id={`${form.service_type.store}`}>
        <ModalBody>
          <div className="row">
