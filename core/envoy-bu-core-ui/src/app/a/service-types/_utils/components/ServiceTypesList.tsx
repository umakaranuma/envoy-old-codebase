...
import { fetchServiceTypeTableData } from '../services';
...
function ServiceTypesList({ tableVers, onView, onEdit, handleOnDelete }: { tableVers: number; onView: Function; onEdit: Function; handleOnDelete: Function }) {
  ...
  const columns = useMemo<ITablePropertyColumn[]>(
    () => [
      ...
      {
        id: 'standardfee',
        header: `${t('standard_fee')} (${currency.code})`,
  ...
  const tableProperties = useAsyncTable({
    ...
    loadData: fetchServiceTypeTableData,
    paginate: true,
    rowSelection: true,
    rowSelectionProp: {
      key: 'id',
  ...
  return (
    <>
      <div className={`data-table-container card custom-card ${isFullscreen ? 'dtc-fullscreen card-fullscreen' : 'mt-4'}`}>
        <Table heading={<PageHeading title={t('service_types')} icon="sun-light" />} {...{ tableProperties, isFullscreen, setIsFullscreen, setIsCustColumnVisible }} />
      </div>
      <CustomizeColumn
        key={tableColumnVers}
