...
import { initFormData, IOpportunityType } from '../model';
import { getOneType, updateType } from '../api-service';
import { InputSkeleton } from '@/components/others/InputSkeleton';
import { useTrans } from '@/helpers/services/lang/langService';
import { clearError, printError } from '@/helpers/handlers/validationErrorHandler';

export const ProductCategoriesEdit = ({ isOpen, editId, afterUpdate, onCancel }: { isOpen: boolean; editId: string; onCancel: Function; afterUpdate: Function }) => {
  const t = useTrans('label.product_categories,otr.common,be.msg');
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responseData = await getOneType(editId);
      console.log(responseData, 'responseData');

      if (responseData?.is_success) {
        const data: IOpportunityType = responseData.result;
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const responseData = await updateType(editId, formData);
      setIsFormProcessing(false);

      if (responseData.status_code === 417) {
        printError(responseData.result, form.type_crud.update, tBe);
      }
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalHeader title={t('edit_entity', { entity: t('type') })} onClose={() => onCancel()} />
      <form onSubmit={onSubmit} id={`${form.type_crud.update}`}>
        <ModalBody>
          <div className="row">
