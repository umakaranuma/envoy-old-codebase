  ...
  return (
    <div className="panel">
      <div className="d-flex justify-content-between mb-3">
        <div className="fs-18 fw-semibold mb-4">{t('my_quotations')}</div>
        {/* <Button color="primary" className="d-flex align-items-center gap-1" onClick={() => setIsOpen(true)}>
        ...
        <Button
          ...
          onClick={() => {
            // setCommercialLine((prevData) => ({ ...prevData, isOpen: true }))
      ...
      <MyQuotationList />
      {/* <SelectRiskType isOpen={isOpen} onCancel={() => setIsOpen(false)} /> */}
      {isOpen && (
        <SelectInterestedProduct
          ...
          setIds={(id: any, productId: any, riskTypeId: any) => {
            router.push(`/${appId}/a/my-quotations/create?fId=${id}&pId=${productId}&rId=${riskTypeId}`, { scroll: false }), setIsOpen(false);
          }}
          key={key}
        />
      )}
