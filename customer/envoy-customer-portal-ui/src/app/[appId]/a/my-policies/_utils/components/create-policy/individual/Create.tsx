...
import PolicyHolderInfo from './forms/PolicyHolderInfo';
import CoverageInfo from './forms/CoverageInfo';
...
import CreatePolicyFormTemplate from './forms/CreatePolicyFormTemplate';
import { createPolicyFormTemplate } from '../../../api-service';
import ReviewAndSubmit from './forms/ReviewAndSubmit';
...
function Create({ formId, productId, riskTypeId }: { formId: string; productId: string; riskTypeId: string }) {
  ...
  useEffect(() => {
    ...
    const requestId = searchParams.get('reqId') || null;
  ...
  const toggleTableTab = (activeTab: string) => {
    ...
    router.push(`/${appId}/a/my-policies/create?t=${activeTab}&fId=${formId}&pId=${productId}&rId=${riskTypeId}&reqId=${requestId}`, { scroll: false });
  };
      ...
      {templateData ? (
        <div className="panel">
          {activeTab === 'personal_info' && (
            <PolicyHolderInfo setToggleTab={(tab: string) => handleToggleStaticForm(tab)} requestId={requestId} onBack={() => handleToggleStaticForm('risk_info')} type="policy" />
          )}
          {activeTab === 'coverage_info' && <CoverageInfo setToggleTab={(tab: string) => handleToggleStaticForm(tab)} requestId={requestId} type="policy" />}
          {activeTab === 'payment_info' && <PaymentInfo setToggleTab={(tab: string) => handleToggleStaticForm(tab)} requestId={requestId} type="policy" />}
          {activeTab === 'supporting_documents' && <SupportingDocuments setToggleTab={(tab: string) => handleToggleStaticForm(tab)} requestId={requestId ? requestId : ''} type="policy" />}
          ...
          {activeTab === 'risk_info' && (
            <CreatePolicyFormTemplate
              ...
              onBack={() => router.push(`/${appId}/a/my-policies`)}
          ...
          {activeTab === 'review_and_submit' && <ReviewAndSubmit setToggleTab={(tab: string) => handleToggleStaticForm(tab)} requestId={requestId ? requestId : ''} type={'policy'} />}
        </div>
      ) : (
