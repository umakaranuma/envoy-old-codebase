...
import { getReviewIInfo, submitPolicyInfo } from '../../../../api-service';
import { Description } from '@/components/others/Description';
import { IReviewInfo } from '../../../../model';
import { useParams, useRouter } from 'next/navigation';
import { toaster } from '@/helpers/services/toaster';
import { thousandSeparator } from '@/helpers/services/commonService';

function ReviewAndSubmit({ setToggleTab, requestId, type }: { setToggleTab: Function; requestId: string | null; type: string }) {
  const t = useTrans('label.my_policy,otr.common');
  // const tBe = useTrans('be.msg,be.error,be.attri');
  const [isFormProcessing, setIsFormProcessing] = useState(false);
  const [skeleton, setSkeleton] = useState(false);
  const [data, setData] = useState<IReviewInfo>({} as IReviewInfo);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responseData = await getReviewIInfo(requestId as string);
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (responseData.is_success) {
        ...
        if (type === 'policy') {
          router.push(`/${appId}/a/my-policies`);
        } else {
          router.push(`/${appId}/a/my-quotations`);
        }
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="d-flex justify-content-start gap-2 mt-3">
        <Button color="light" className="d-flex align-items-center gap-1" onClick={() => setToggleTab('terms_and_conditions')}>
