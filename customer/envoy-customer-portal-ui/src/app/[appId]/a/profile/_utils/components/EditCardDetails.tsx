...
function EditCardDetails({ isOpen, onCancel }: { isOpen: boolean; onCancel: Function }) {
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalHeader title={t('edit_card_details')} onClose={() => onCancel()} />
      <ModalBody>
        <div id={`${form.card_details.store}`}>
          <div className="row">
            ...
            <div className="col-12 mb-3">
              <Label label={t('name_on_the_card')} isRequired />
              {skeleton ? (
                <InputSkeleton />
              ) : (
                <Input value={formData.name_on_the_card} className="form-control error-name_on_the_card" name="name_on_the_card" onChange={(e) => onFormChange('name_on_the_card', e.target.value)} />
              )}
            </div>
            <div className="col-12 mb-3">
              <Label label={t('card_number')} isRequired />
              {skeleton ? (
                <InputSkeleton />
              ) : (
                <Input value={formData.card_number} className="form-control error-card_number" name="card_number" onChange={(e) => onFormChange('card_number', e.target.value)} />
              )}
            </div>
            <div className="col-12 col-md-6 mb-3">
              <Label label={t('expiry_month_and_year')} isRequired />
              {skeleton ? (
                <InputSkeleton />
              ) : (
                <Input
            ...
            <div className="col-12 col-md-6 mb-3">
              <Label label={t('cvv')} isRequired />
              {skeleton ? <InputSkeleton /> : <Input value={formData.cvv} className="form-control error-cvv" name="cvv" onChange={(e) => onFormChange('cvv', e.target.value)} placeholder="CVV" />}
            </div>
          </div>
        </div>
      </ModalBody>
