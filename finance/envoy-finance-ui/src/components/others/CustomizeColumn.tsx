...
export const useCustomizeColumn = ({ columns, tableName, tableColumnVers }: { columns: ITablePropertyColumn[]; tableName: string; tableColumnVers: any }) => {
  const [tableColumns, setTableColumns] = useState([] as ITablePropertyColumn[]);

  useEffect(() => {
    const setData = async () => {
      try {
        const db = new Localbase('tc_config'); // Initialize indexDB database
        ...
        const updatedColumns = columns.map((column: ITablePropertyColumn, index: number) => {
          ...
          return {
            ...column,
            visibilityLock: column.visibilityLock === false ? false : true, // Lock visibility if not already false
            customizable: column.customizable === false ? false : true, // Lock customization if not already false
            isHidden: existingData ? existingData.is_hidden : column.isHidden === true ? true : false, // Set hidden status based on existing data
            order: existingData ? existingData.order : index, // Set order from existing data or default to index
          };
        });
