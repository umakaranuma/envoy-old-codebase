...
export async function getHealthByOfOpportunity(id: string, health_id: string) {
  return responseHandling(
    await sendRequest({
      url: `${process.env.CRM_PROXY_PREFIX}/api/opportunities/${id}/health/${health_id}`,
      method: 'GET',
...
export async function getAllOpportunityTypeFormAttributes(opportunityTypeId: string, data_gethering_type: 'ONBOARDING') {
  const response = await sendRequest({
