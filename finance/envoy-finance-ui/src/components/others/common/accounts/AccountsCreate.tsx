...
function AccountsCreate({ isOpen, onCancel, afterSave, existingSelection = false }: { isOpen: boolean; onCancel: Function; afterSave: Function; existingSelection?: boolean }) {
  const t = useTrans('label.accounts,otr.common,label.risks');
    ...
    try {
      if (createType === 'exist') {
        ...
        if (responseData?.status_code === 417) {
          printError(responseData.result, form.sample_crud_form.store, tBe);
          return;
        }
      } else {
        ...
        if (cResponse.status_code === 417) {
          printError(cResponse.result, form.sample_crud_form.store, tBe);
          return;
        }
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    clearError(form.sample_crud_form.store);
    clearError(form.sample_crud_form.store);
    clearError(form.sample_crud_form.store);
    clearError(form.sample_crud_form.store);
  ...
  return (
    <Modal isOpen={isOpen} size="lg">
      ...
      {/* <form onSubmit={onSubmit} id={`${form.sample_crud_form.store}`}> */}
      <ModalBody>
        {existingSelection && (
        ...
        {formType === 'select-from-existing' ? (
          <form id={`${form.sample_crud_form.store}`}>
