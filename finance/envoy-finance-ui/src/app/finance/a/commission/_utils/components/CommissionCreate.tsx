...
function CommissionCreate({ isOpen, onCancel, afterSave }: { isOpen: boolean; onCancel: Function; afterSave: Function }) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    clearError(form.commission_crud.store);
    ...
    try {
      ...
      if (responseData.status_code === 417) {
        printError(responseData.result, form.commission_crud.store, tBe);
      }
  ...
  return (
    <Modal isOpen={isOpen}>
      ...
      <form onSubmit={onSubmit} id={`${form.commission_crud.store}`}>
        <ModalBody>
          <div className="row">
