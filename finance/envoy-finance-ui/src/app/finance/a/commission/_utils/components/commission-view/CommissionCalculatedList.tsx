...
import { fetchOneCommissionHistoryCalculatedData } from '../../services';
import { useParams } from 'next/navigation';

function CommissionCalculatedList() {
  ...
  const tableProperties = useAsyncTable({
    columns: columns,
    loadData: (params) => fetchOneCommissionHistoryCalculatedData(params, commissionId),
    paginate: true,
    rowSelection: false,
    // rowSelectionProp: {
    //   key: 'id',
  ...
  return (
    <div className={`data-table-container card custom-card mt-2`}>
      ...
      <Table heading={<PageHeading title={t('commission_calculated')} icon="sun-light" />} {...{ tableProperties, searchOption: false }} />
    </div>
  );
}

export default CommissionCalculatedList;
