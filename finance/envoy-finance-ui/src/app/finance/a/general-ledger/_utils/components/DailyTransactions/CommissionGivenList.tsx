...
import { fetchCommissionGivenTableData } from '../../service';
...
function CommissionGivenList({ tableVers }: { tableVers: number; onView: Function; onEdit: Function; handleOnDelete: Function }) {
  const t = useTrans('label.general_ledger,otr.common');
  const tableName = 'commission_given_summary';
  ...
  const tableProperties = useAsyncTable({
    ...
    loadData: async (params) => {
      const response = await fetchCommissionGivenTableData(params);
      if (response?.data) {
      ...
      <div className="row">
        ...
        <div className="col-md-8">
          <div className={`data-table-container card custom-card ${isFullscreen ? 'dtc-fullscreen card-fullscreen' : ''}`}>
            <Table heading={<PageHeading title={t('commission_given')} icon="sun-light" />} {...{ tableProperties, isFullscreen, setIsFullscreen, setIsCustColumnVisible, setIsFilterVisible }} />
          </div>
        </div>
...
export default CommissionGivenList;
