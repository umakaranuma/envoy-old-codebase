...
def handle_entity_docs(entity_id, docs, is_update=False):
    print(f"DEBUG: handle_entity_docs called with entity_id={entity_id}, docs={docs}, is_update={is_update}")
    
    if not docs:
        print("DEBUG: No docs provided, returning None")
        return None
    ...
    if is_update:
        updated_rows = []
        for doc in docs:
            print(f"DEBUG: Updating document: {doc}")
            updated = (
        ...
        print(f"Updated documents: {updated_rows}")
        return updated_rows
    else:
        inserted_rows = []
        for doc in docs:
            print(f"DEBUG: Inserting document: {doc}")
            doc_data = {
                "entity_id": entity_id,
                "doc": doc.get("doc") or "",  # Ensure doc is never None
                "name": doc.get("name") or "",  # Ensure name is never None
                "type": doc.get("type") or "",  # Ensure type is never None
            }
            print(f"DEBUG: Document data to insert: {doc_data}")
            
            # Only insert if we have at least a name or doc content
            if doc_data["name"] or doc_data["doc"]:
                inserted = qb.insert(doc_data)
                print(f"Inserted document: {inserted}")
                inserted_rows.append(inserted)
            else:
                print(f"DEBUG: Skipping document insertion - no name or doc content")
                inserted_rows.append(None)
...
def _format_date_fields(record):
    ...
    Preserves full datetime format (with time) for 'created_at' and 'updated_at'.
    """
    # Fields that should preserve full datetime format (with time)
    preserve_datetime_fields = ['created_at', 'updated_at']
    ...
    for key, value in list(record.items()):
        ...
        # If ts is now a datetime, format appropriately
        if isinstance(ts, datetime):
            # Preserve full datetime format for created_at and updated_at
            if key in preserve_datetime_fields:
                record[key] = ts.strftime("%Y-%m-%d %H:%M:%S")
            else:
                # Format date-only for other fields
