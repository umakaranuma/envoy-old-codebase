...
def _bulk_create(request, policy_type):
    ...
    # Validate the main payload structure using ValidatorService
    bulk_rules = {
        'documents': 'required|array|min:1',
        cfg['id_field']: f"required|integer|exists:{cfg['exists_table']},id"
    }
    
    if errors := ValidatorService.validate(payload, bulk_rules):
        return ResponseService.response('VALIDATION_ERROR', errors, Error.VALIDATION_ERROR)
...
@csrf_exempt
@api_view(['GET'])
def documents_by_issued_policy_id(request, issued_policy_id):
    """
    Get both policy and risk documents for an issued policy by fetching from the related request policy.
    Uses QueryBuilderService without complex JSON aggregation.
    """
    # Authorization
    if resp := _authorize(request, 'VIEW'):
        return resp

    # First, get the request_policy_id from the issued_policy_id
    issued_policy = QueryBuilderService('crmp_issued_policies')\
        .select('policy_request_id')\
    ...
    request_policy_id = issued_policy.get('policy_request_id')
    if not request_policy_id:
        return ResponseService.response('NOT_FOUND', None, Error.NOT_FOUND)

    # Get request policy basic info
    request_policy = QueryBuilderService('crmp_request_policies')\
        .select('id', 'policy_request_id', 'policy_base_id')\
    ...
    policy_base_id = request_policy.get('policy_base_id')
    if not policy_base_id:
        return ResponseService.response('NOT_FOUND', None, Error.NOT_FOUND)

    # Get all documents for this request policy
    documents = QueryBuilderService('crmp_policy_documents')\
        .select(
            'core_product_document_types.id as document_type_id',
            'crmp_policy_documents.value',
            'core_product_document_types.name as document_name',
            'core_product_document_types.type as document_type'
        )\
        .leftJoin('core_product_document_types', 'core_product_document_types.id', 'crmp_policy_documents.document_type_id')\
    ...
    for doc in documents:
        doc_data = {
            'id': doc['document_type_id'],  # Use document_type_id instead of document record id
            'value': doc['value'],
            'document_name': doc['document_name']
        }
        
        if doc['document_type'] == 'policy':
            policy_documents.append(doc_data)
        elif doc['document_type'] == 'risk':
            risk_documents.append(doc_data)

    # Build response
    response_data = {
        'request_policy_id': request_policy_id,
        'policy_request_code': request_policy.get('policy_request_id'),
        'issued_policy_id': issued_policy_id,
        'policy_document_value': policy_documents,
        'risk_document_value': risk_documents
    }
