...
@csrf_exempt
@api_view(['GET', 'POST'])
def get_form_config_info(request, policy_request_id, config_id):
    if request.method == 'GET':
        ...
        return getAll(request, policy_request_id, config_id)
    elif request.method == 'POST':
        action = ActionService.getAction("Policy_Type", "Create")
        has_authority = AuthService.hasAuthority(request, action)
        if not has_authority:
            return ResponseService.response('FORBIDDEN', None, Error.UN_AUTHORIZED)
        return save_policy_form_submission(request, policy_request_id, config_id)


def getAll(request, policy_request_id, config_id):
    data = QueryBuilderService("crmp_policy_risk_reg_form_submissions") \
            .where('policy_request_id', policy_request_id) \
...
def save_policy_form_submission(request, policy_request_id, config_id):
    # Step 1: Fetch form configuration
    form_config = QueryBuilderService("crmp_claim_form_config").where('id', config_id).first()
    if not form_config:
        return ResponseService.response('NOT_FOUND', None, Error.NOT_FOUND)

    # Step 2: Check if config_id exists in the expected FK table (crm_opportunity_form_config)
    fk_check = QueryBuilderService("crm_opportunity_form_config").where("id", form_config["id"]).first()
    if not fk_check:
        return ResponseService.response('VALIDATION_ERROR', None, f"Invalid oppor_form_config_id: {form_config['id']} does not exist.")
    ...
    if isinstance(data, list):
        ...
        return ResponseService.response('VALIDATION_ERROR', None, "Invalid request payload format.")
    ...
    # Step 5: Insert mapping into crmp_policy_risk_reg_form_submissions
...
def get_single_form_config_info(request, policy_request_id, config_id, info_id):
    if request.method == 'GET':
...
@csrf_exempt
@api_view(['POST'])
def clone_oppo_submissions(request, policy_request_id):
    data = json.loads(request.body)
    ...
    for opp_id in opp_ids:
        ...
        for row in original_rows:
            ...
            new_row["policy_request_id"] = policy_request_id
...
@csrf_exempt
@api_view(['GET'])
def getAll_existing_risk(request,config_id):
