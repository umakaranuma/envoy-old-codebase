...
function PolicyInfo({ setIsFormTemplateVisible, onBack, policyId }: { setIsFormTemplateVisible: Function; onBack: Function; policyId: string }) {
  const t = useTrans('label.claim,label.issued_policies,otr.common');
  ...
  return (
    <>
      <div>
        ...
        <div className="panel">
          <div className="panel-title mb-3">{t('policy_info')}</div>
          <div className="row">
            <div className="col-12 col-md-3 mb-3">
        ...
        {/* <div className="panel">
          ...
          {skeleton ? (
            ...
            <>
              {data.risk_info &&
                data.risk_info.map((risk) => (
                  <div className="row mb-2" key={risk.risk_id}>
                    <div className="panel-subtitle">{risk.risk_type_title}</div>
                    {risk.submission_values.map((value, idx) => (
                      <div className="col-12 col-md-3 mb-3" key={idx}>
                        <Description label={value.label || '-'} value={value.value || '-'} skeleton={skeleton} />
                      </div>
                    ))}
                  </div>
                ))}
