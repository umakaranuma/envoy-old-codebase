...
import { getOneEvaluation } from '@/app/policy/a/claim/_utils/api-service';
import { IElement, IFormTemplate, Step } from '@/components/others/common/form/template-modal';
...
function FormTemplateEvaluationView({ claimId, onBack, currentPath, handleNextPage }: { claimId: string; onBack: Function; currentPath: string; handleNextPage?: Function }) {
  ...
  const fetchTemplateData = async () => {
    if (claimId) {
      ...
      // if (responseData.system_code === '404') {
                              ...
                              const value =
                                element.code === 'MULTI_SELECT' || element.code === 'MULTI_CHOICE'
                                  ...
                                  : element.code === 'CURRENCY' && element.value
                                    ? `amount: ${thousandSeparator(JSON.parse(element.value.replace(/'/g, '"')).amount)}, currency: ${JSON.parse(element.value.replace(/'/g, '"')).currency}`
                                    : element.value;
                              return (
...
export default FormTemplateEvaluationView;
