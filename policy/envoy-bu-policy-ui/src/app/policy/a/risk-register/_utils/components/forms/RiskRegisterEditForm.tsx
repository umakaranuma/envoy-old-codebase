...
function RiskRegisterEditForm({ riskId, onBack, isFormProcessing, onSubmit, currentPath }: { riskId: string; onBack: Function; isFormProcessing: boolean; onSubmit: Function; currentPath: string }) {
              ...
              templateData.panels.map((panel, index) => (
                <div key={panel.id} className={templateData.template?.type === 'multi_step_form' ? `d-${currentTab.id && currentTab.id === panel.step_id ? 'block' : 'none'}` : 'd-block'}>
                  <div className="card-body bg-white p-3 rounded-3 mb-3" key={index}>
                    <div className="panel-title-sm">{panel.title}</div>
                    {
                      <div className="row">
                        {formData
