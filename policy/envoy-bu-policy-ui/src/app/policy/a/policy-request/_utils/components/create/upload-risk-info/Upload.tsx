...
function Upload({ backUrl, title, mappingFields, onFileUpload, onMappingSubmit }: UploadProps) {
    ...
    if (currentTabId === 3) {
      ...
      try {
        ...
        if (response.is_success || response.success) {
          ...
          // router.push(`/crm/a/sales-managements/${100}?t=opp-type&f=board`);
        } else {
  ...
  return (
    <>
      <GoTo goTo={() => router.push(backUrl)} title={title} />
      ...
      {currentTabId === 3 && <PreviewTable rows={previewRows} fields={mappingFields} onEditRow={() => {}} handleDeleteRow={handleDeleteRow} />}
