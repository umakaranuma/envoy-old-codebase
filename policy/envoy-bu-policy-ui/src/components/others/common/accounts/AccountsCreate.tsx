  ...
  const saveCustomerContact = async (custId: string, callback: any) => {
    ...
    try {
      ...
      if (responseData?.is_success) {
        callback();
        onCreatedCustomer({ name: formData.name, id: responseData.result.customer_id, email: responseData.result?.contact?.email, primary_contact: responseData.result?.contact?.primary_contact });
      }
    } catch (error) {
          ...
          <>
            <form id={`${form.customres_crud.store}`}>
              ...
              <div className="row">
                ...
                <div className="col-12 col-md-6 mb-3">
                  <Input label={t('email')} value={formData.email} isRequired onChange={(e) => onFormChange('email', e.target.value)} className="form-control error-email" name="email" />
                </div>
                <div className="col-12 col-md-6 mb-3">
