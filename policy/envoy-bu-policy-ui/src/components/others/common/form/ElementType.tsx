    ...
    case 'SUBMISSION_PICKER':
      return (
        <FilePreviewer
          ...
          isRequired={isRequired}
          className={`error-${elementId}`}
          initialUrl={getInitialUrl(value)}
        />
      );
    case 'LOCATION':
      return label && <Label label={label} isRequired={isRequired} />;
      ...
      return (
        <div>
          ...
          <video width="100%" controls className="rounded">
            <source src={process.env.S3CDN + '/' + value} />
          </video>
        </div>
      );
    case 'PDF_VIEWER': {
...
const getInitialUrl = (value: any): string | undefined => {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      return `${process.env.S3CDN}/${parsed[0]}`;
    }
  } catch (error) {
