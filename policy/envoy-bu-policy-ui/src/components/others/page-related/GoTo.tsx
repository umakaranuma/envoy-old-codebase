...
function GoTo({ goTo, title, skeleton }: { goTo: Function; title: string; skeleton?: boolean }) {
  return (
    <>
      <div className="d-flex align-items-center gap-2 mb-4 page-go-back">
        ...
        {!skeleton ? <div className="fs-15 fw-medium ms-2">{title}</div> : <Skeleton height="24px" width="200px" />}
      </div>
    </>
  );
}
