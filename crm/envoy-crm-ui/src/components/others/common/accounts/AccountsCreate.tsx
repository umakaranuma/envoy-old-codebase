...
import { createContact, createCustomerContacts, createCustomers } from './api-service';
import { useTrans } from '@/helpers/services/lang/langService';
import { clearError, printError } from '@/helpers/handlers/validationErrorHandler';
import { AsyncSelect } from '@apptimus-ui/select';
import { fetchAllCustomers, fetchContacts } from './services';
import PhoneInput from 'react-phone-input-2';
import 'react-phone-input-2/lib/style.css';
import { FlexField, useFlexField } from '../../FlexFiled';
...
function AccountsCreate({
  ...
  populateData?: { accType: string; contactNumber: string; email: string; contactId: string; contact: string; name: string };
}) {
  const t = useTrans('label.accounts,otr.common,label.sales_managements');
  ...
  const [formType, setFormType] = useState<'create-new' | 'select-from-existing'>('create-new');
  ...
  const [defaultContactData, setDefaultContactData] = useState({ id: '', name: '' });
  const [isContactNotMatched, setIsContactNotMatched] = useState(false);

  useEffect(() => {
    if (populateData) {
      setFormData((prevFormData) => ({
        ...prevFormData,
        type: populateData.accType,
        primary_contact: populateData.contactNumber,
        email: populateData.email,
      }));
      setDefaultContactData({ id: populateData.contactId, name: populateData.contact });
      onFormChange('name', populateData.name);
      onContactFormChange('name', populateData.contact);
      onContactFormChange('email', populateData.email);
      onContactFormChange('primary_contact', populateData.contactNumber);
    }
  }, [populateData]);

  useEffect(() => {
    if (formData.primary_contact !== contactData.primary_contact) {
      setIsContactNotMatched(true);
    } else {
      setIsContactNotMatched(false);
    }
    console.log('formData.primary_contact', formData.primary_contact);
    console.log('contactData.primary_contact', contactData.primary_contact);
  }, [formData.primary_contact, contactData.primary_contact]);
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    clearError(form.select_customer.store);
    ...
    try {
      if (createdCustomerId === '') {
        ...
        if (formType === 'create-new') {
          responseData = await createCustomers(formData);
        } else {
          if (existingAccount) {
            afterSave(existingAccount);
          } else {
            if (existingAccount === '') {
              error['account_id'] = [
                {
            ...
            if (Object.keys(error).length > 0) {
              ...
              printError(error, form.select_customer.store, tBe);
              return;
            }
          }
        }
        ...
        if (responseData && responseData.is_success) {
          ...
          if (formType === 'create-new') {
            saveCustomerContact(formType === 'create-new' ? responseData.result.id : existingAccount, () => toaster.success(tBe(responseData.message), afterSave(responseData.result.id)));
          } else {
            toaster.success(tBe(responseData.message), afterSave(responseData.result.id));
          }
          setFinalResponse(responseData);
        }
      } else {
        if (formType === 'create-new') {
          saveCustomerContact(createdCustomerId, () => toaster.success(tBe(finalResponse && finalResponse.message), afterSave(finalResponse.result.id)));
        }
      }
    } catch (error) {
    ...
    <Modal isOpen={isOpen} size="lg" scrollable={true}>
      ...
      <ModalBody>
        ...
        {formType === 'select-from-existing' ? (
          <form id={`${form.select_customer.store}`} style={{ minHeight: '200px' }}>
            <div className="col-12 col-md-6 mb-3 custom-select custom-dropdown compare-criteria">
              <Label htmlFor="account" label={t('account')} isRequired />
              <AsyncSelect
                onChange={(value) => setExistingAccount(value)}
                className="form-control error-account_id"
                option={{
                  labelFn: (option) => (
                    <>
                      <div className="text">{option.name}</div>
                      <div className="d-flex flex-row justify-content-between align-items-center gap-2 mt-1">
                        <div className="text-muted fs-14">{option.primary_contact_email}</div>
                        <div className="text-muted fs-13">{option.primary_contact_number}</div>
                      </div>
                    </>
                  ),
                  label: 'name',
          ...
          <>
            <form id={`${form.customres_crud.store}`}>
              <>
                <div className="col-12 col-md-6 mb-3">
                  ...
                  {/* <Select
                    ...
                    defaultValue={populateData?.accType ? { label: populateData.accType, value: populateData.accType } : null}
                    option={{ label: 'label', value: 'value' }}
                    isSearchable={false}
                    className="form-control error-type"
                  /> */}
                  <Input disabled value={formData.type || ''} onChange={(e) => onFormChange('type', e.target.value)} className="form-control error-type" />
                </div>
                <div className="mb-3">
                  <input
                    type="checkbox"
              ...
              <div className="row">
                ...
                <div className="col-12 col-md-6 mb-3">
                  <Input label={t('email')} isRequired value={formData.email || ''} onChange={(e) => onFormChange('email', e.target.value)} className="form-control error-email" name="email" />
                </div>
                <div className="col-12 col-md-6 mb-3">
                ...
                <div className="col-12 col-md-6 mb-3">
                  ...
                  {isContactNotMatched && <span className="text-warning fs-13">{t('primary_contact_number_not_matched')}</span>}
                </div>
                <div className="col-12 col-md-6 mb-3">
                ...
                <div className="mb-3">
                  <input
                    type="checkbox"
                    checked={formData.is_configure === 1}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      onFormChange('is_configure', checked ? 1 : 0);
                    }}
                  />
                  <span className="ms-2 fs-14">{t('config_customer_portal_account')}</span>
                </div>
              </div>
            </form>
            <div className="mt-3">
                  ...
                  <form id={`${form.select_contact.store}`}>
                    <div className="col-12 col-md-6 mb-3 custom-select">
                      ...
                      <AsyncSelect
                        ...
                        className="form-control error-contact_id custom-dropdown"
                        option={{
                          labelFn: (option) => (
                            <>
                              <div className="text">{option.name}</div>
                              <div className="text-muted fs-13">{option.primary_contact}</div>
                            </>
                          ),
                          label: 'name',
                          value: 'id',
                        ...
                        defaultValue={defaultContactData}
                      />
                    </div>
                  </form>
                ) : (
                  <>
                    <form id={`${form.add_contact.store}`}>
                      <div className="row">
                        ...
                        <div className="col-12 col-md-6 mb-3">
                          <Input label={t('email')} value={contactData.email || ''} onChange={(e) => onContactFormChange('email', e.target.value)} className="form-control error-email" name="email" />
                        </div>
                        <div className="col-12 mb-3">
                          <Input
                            label={t('remarks')}
