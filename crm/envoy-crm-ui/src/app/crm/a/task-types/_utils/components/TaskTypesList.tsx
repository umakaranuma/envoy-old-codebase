...
import { fetchTaskTypesTableData } from '../services';

function TaskTypesList({ tableVers, onView, onEdit, handleOnDelete }: { tableVers: number; onView: Function; onEdit: Function; handleOnDelete: Function }) {
  ...
  const tableProperties = useAsyncTable({
    ...
    loadData: fetchTaskTypesTableData,
    paginate: true,
    rowSelection: true,
    rowSelectionProp: {
      key: 'id',
  ...
  return (
    <>
      <div className={`data-table-container card custom-card ${isFullscreen ? 'card-fullscreen table-header-freeze' : 'mt-4'}`}>
        <Table heading={<PageHeading title={t('task_types')} icon="sun-light" />} {...{ tableProperties, isFullscreen, setIsFullscreen, setIsCustColumnVisible }} />
      </div>
      <CustomizeColumn
        key={tableColumnVers}
