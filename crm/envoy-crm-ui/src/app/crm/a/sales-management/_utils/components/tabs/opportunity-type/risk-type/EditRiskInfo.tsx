...
import { getOneRiskInfo, updateOneRiskInfo } from '../../../../api-service';
...
function EditRiskInfo({ isOpen, onCancel, afterSave, riskInfoId }: { isOpen: boolean; onCancel: Function; afterSave: Function; riskInfoId: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getOneRiskInfo(riskInfoId);
  ...
  async function onSubmit(data: IElements[]) {
    ...
    try {
      const responseData = await updateOneRiskInfo(riskInfoId, {
