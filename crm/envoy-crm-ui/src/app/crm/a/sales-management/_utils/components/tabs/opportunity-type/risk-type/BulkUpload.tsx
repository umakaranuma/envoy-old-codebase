...
import { bulkUpload, getBulkUploadExcel } from '../_utils/api-service';
import { form } from '@/constans/Form';
import { clearError, printError } from '@/helpers/handlers/validationErrorHandler';

function BulkUpload({ isOpen, onCancel, afterSave, riskTypeIds, leadId }: { isOpen: boolean; onCancel: Function; afterSave: Function; riskTypeIds: string[]; leadId: string }) {
  const [isFormProcessing, setIsFormProcessing] = useState(false);
  const t = useTrans('label.policy_request,otr.common,be.msg');
  const tBe = useTrans('be.msg,be.error,be.attri');

  const [loading, setLoading] = useState(true);
  const [formData, setFormData] = useState({ excelFile: '', file_key: '', lead_id: '' });
