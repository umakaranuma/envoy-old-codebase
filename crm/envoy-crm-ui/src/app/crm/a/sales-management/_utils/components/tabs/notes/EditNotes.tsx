...
import { getOneNotes, updateNotes } from '../../../api-service';
...
function EditNotes({ isOpen, editId, afterUpdate, onCancel, entityId }: { isOpen: boolean; editId: string; onCancel: Function; afterUpdate: Function; entityId: string }) {
  ...
  const [formData, setFormData] = useState(initNotesFormData);
  ...
  return (
    <Modal isOpen={isOpen}>
      <ModalHeader title={t('edit_notes')} onClose={() => onCancel()} />
          ...
          <div className="row">
            <div className="col-12 col-md-6 mb-3">
              ...
              {skeleton ? (
                ...
                <Input type="text" value={formData.notes} onChange={(e) => onFormChange('notes', e.target.value)} className="form-control error-notes" id="notes" name="notes" />
              )}
            </div>
