...
import { getProductDocuments } from '../../../../policy-api-service';
import { IIDocument, IProductDocument } from '../../../../model';

const ProductDocuments = forwardRef(({ productId, productType }: { productId: string; productType: string }, ref) => {
  const t = useTrans('label.policy_request,otr.common');
  const tBe = useTrans('be.msg,be.error,be.attri');
  const [documents, setDocuments] = useState<IProductDocument[]>([]);
  const [skeleton, setSkeleton] = useState(false);
  const [formData, setFormData] = useState({} as IIDocument);
                    ...
                    {documents.filter((doc) => doc.type === 'policy').length > 0 && (
                      <div className="row mb-3">
                        ...
                        {documents
                          ...
                          .map((doc) => (
                            <div key={doc.id} className="col-12 col-md-6 mb-2 d-flex flex-column">
