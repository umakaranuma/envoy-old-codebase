...
// import { IEmailDocument } from '../../model';
...
//         emailData({ ...formData, files: fileData });
//       } else {
//         emailData(formData);
//       }
//     }
...
//       const key = await fileUploader(s3FormData, 'crm/quotations');
//       const fileType = file.name.split('.').pop();
//       const fileName = file.name;
//       fileData.push({ doc_link: key, doc_name: fileName, doc_type: fileType });
//     }
...
//     const updatedDocuments = formData.defaultDocuments.filter((doc: IEmailDocument) => doc.coverage_details_name !== name);
...
//             {formData.defaultDocuments && formData.defaultDocuments?.length > 0 && (
//               <div className="d-flex flex-column gap-2">
//                 {formData.defaultDocuments.map((file: IEmailDocument, key) => (
//                   <FilePreviewInput key={key} fileName={file.coverage_details_name} onCancel={() => handleRemoveDocument(file.coverage_details_name)} />
//                 ))}
//               </div>
//             )}
//             {files.length > 0 && (
...
//           <Button text={t('send_email')} onClick={onSubmit} width="sm" isLoading={isFormProcessing} />
