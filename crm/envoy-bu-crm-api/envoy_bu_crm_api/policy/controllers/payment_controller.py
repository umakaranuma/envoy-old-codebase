...
from .invoice_utils import update_invoice_payment_details
...
def create_payment(request):
    ...
    if invoice.get("issued_policy_id"):
        policy = QueryBuilderService("crmp_issued_policies")\
                      .select("paid_amount")\
                      .where("id", invoice["issued_policy_id"]).first()
        raw_paid = policy.get("paid_amount") if policy else None

        try:
            current_paid = Decimal(str(raw_paid or "0.00"))
        ...
        QueryBuilderService("crmp_issued_policies")\
            .where("id", invoice["issued_policy_id"])\
